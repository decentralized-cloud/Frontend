const GenerateNodeDeploymentScript = (ip: string, port: string, token: string): string => {
  const result = `
#!/usr/bin/env bash
...
K3S_CLUSTER_IP=${ip}
K3S_CLUSTER_PORT=${port}
K3S_CLIENT_TOKEN=${token}
...
export { GenerateNodeDeploymentScript };
